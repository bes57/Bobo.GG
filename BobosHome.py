...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .cards { display:flex; gap:20px; margin-top:52px; flex-wrap:wrap; justify-content:center; }
  .nav-card { background:white; border-radius:24px; padding:32px 36px; width:260px; text-decoration:none; color:var(--ink); box-shadow:0 4px 24px #0000000a; transition:transform .2s,box-shadow .2s; text-align:left; }
  .nav-card:hover { transform:translateY(-6px); box-shadow:0 16px 40px #00000014; }
.nav-card-title { font-family:'Syne',sans-serif; font-size:1.1rem; font-weight:800; margin-bottom:6px; }
...
def favicon():
    return send_from_directory(os.path.dirname(__file__), "BoboLogo-cropped.svg", mimetype="image/svg+xml")

@app.route("/logo.svg")
def logo():
    return send_from_directory(os.path.dirname(__file__), "BoboLogo.svg", mimetype="image/svg+xml")
